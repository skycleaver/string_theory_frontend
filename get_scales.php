    ...
    public function getScale($rootNote, $scaleName) {
        ...
        switch ($scaleName) {
            case self::MAJOR:
                ...
                //$_POST['scale']['chords'][$scale[6] . ' ' . self::DIMINISHED] = $this->getChord->getDiminishedChord($scale[6]);
                break;
            case self::MINOR:
                $scale = $this->getMinorScale($rootNote);
                $_POST['scale']['chords'][$scale[0] . ' ' . self::MINOR] = $this->getChord->getMinorChord($scale[0]);
                //$_POST['scale']['chords'][$scale[1] . ' ' . self::DIMINISHED] = $this->getChord->getDiminishedChord($scale[1]);
                $_POST['scale']['chords'][$scale[2] . ' ' . self::MAJOR] = $this->getChord->getMajorChord($scale[2]);
                $_POST['scale']['chords'][$scale[3] . ' ' . self::MINOR] = $this->getChord->getMinorChord($scale[3]);
                $_POST['scale']['chords'][$scale[4] . ' ' . self::MINOR] = $this->getChord->getMinorChord($scale[4]);
                $_POST['scale']['chords'][$scale[5] . ' ' . self::MAJOR] = $this->getChord->getMajorChord($scale[5]);
                $_POST['scale']['chords'][$scale[6] . ' ' . self::MAJOR] = $this->getChord->getMajorChord($scale[6]);
                break;
            default:
                unset($_POST['scale']);
